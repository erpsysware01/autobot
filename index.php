...
//**********************************************************เลขบัตรประชาชน******************************* */
  ...
  if($response->pament_type == 8 | $response->pament_type == 9 | $response->pament_type == 10 | $response->pament_type == 11 ){
    ...
    if($response->student_name !=null){
      ...
      $QRC_PNG="https://select2web-autobot-bantan.herokuapp.com/LineQrcode/phpqrcode/QRC/BG_QR_2.php?ref1=$response->ref1&&ref2=$response->ref2&&price=$response->price&&payment_type_name=$payment_type_name1&&student_name=$student_name1&&class_name=$class_name1&&school_name=$school_name1";
      $arrPostData = array();
      $arrPostData['replyToken'] = $arrJson['events'][0]['replyToken'];
